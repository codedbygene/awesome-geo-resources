
# Awesome GEO Resources [![Awesome](https://awesome.re/badge.svg)](https://awesome.re)

> A curated list of the best Generative Engine Optimization (GEO) resources, tools, case studies, prompts, research papers, and examples.

GEO (Generative Engine Optimization) is the practice of helping AI systems like ChatGPT, Claude, Gemini, Perplexity, and other AI search engines better understand, reference, and recommend your content, products, and websites.

---

## Contents

* [What is GEO?](#what-is-geo)
* [GEO Fundamentals](#geo-fundamentals)
* [AI Search Engines](#ai-search-engines)
* [GEO Tools](#geo-tools)
* [llms.txt Resources](#llmstxt-resources)
* [AI Crawlers & Indexing](#ai-crawlers--indexing)
* [Structured Data & Schema](#structured-data--schema)
* [Alternative & Comparison Pages](#alternative--comparison-pages)
* [AI-Friendly Content](#ai-friendly-content)
* [GEO Case Studies](#geo-case-studies)
* [Research Papers](#research-papers)
* [GEO Prompts](#geo-prompts)
* [GEO Checklist](#geo-checklist)
* [Communities & Newsletters](#communities--newsletters)

---

# What is GEO?

Generative Engine Optimization (GEO) is the next evolution of SEO.

Instead of optimizing only for Google rankings, GEO focuses on helping AI systems:

* Understand your content
* Reference your website
* Recommend your products
* Surface your expertise

Popular AI search systems include:

* ChatGPT
* Claude
* Gemini
* Perplexity
* Microsoft Copilot

---

# GEO Fundamentals

## Beginner Guides

* GEO vs SEO
* GEO vs AEO
* How AI Search Works
* How AI Models Discover Content
* AI Search Ranking Signals

## Core Concepts

* Entities
* Structured Data
* Knowledge Graphs
* Semantic Search
* Citations
* Freshness Signals

---

# AI Search Engines

## Search Platforms

* ChatGPT Search
* Perplexity
* Gemini
* Claude
* Copilot

## AI Agents

* OpenAI Agents
* Claude Agents
* Browser Agents
* MCP Ecosystem

---

# GEO Tools

## GEO Audit

* GEO Audit Tools
* AI Visibility Checker
* AI Citation Tracker

## Optimization

* FAQ Generators
* Schema Generators
* GEO Score Checkers

## Monitoring

* Brand Mention Tracking
* Competitor Tracking
* Citation Monitoring

---

# llms.txt Resources

## Guides

* What is llms.txt?
* Why llms.txt Matters

## Tools

* llms.txt Generator
* llms.txt Validator
* llms.txt Examples

## Templates

* SaaS Template
* AI Tool Template
* Documentation Template

---

# AI Crawlers & Indexing

## AI Crawlers

* GPTBot
* ClaudeBot
* PerplexityBot
* Google-Extended

## Resources

* AI Robots.txt Guides
* Crawl Monitoring
* Indexability Testing

---

# Structured Data & Schema

## Schema Types

* Organization
* Product
* SoftwareApplication
* FAQPage
* Article

## Resources

* Schema Validators
* JSON-LD Generators
* Rich Result Testing

---

# Alternative & Comparison Pages

One of the most effective GEO strategies.

Examples:

* ChatGPT Alternatives
* Claude Alternatives
* Gamma Alternatives

Comparison Pages:

* ChatGPT vs Claude
* Claude vs Gemini
* Gamma vs Tome

---

# AI-Friendly Content

## Content Formats

* FAQ Pages
* Buying Guides
* Tool Roundups
* Comparison Pages
* Alternative Pages

## Content Structures

* Question-Based Content
* Use-Case Content
* Problem-Solution Content
* Comparison Content

---

# GEO Case Studies

## SaaS GEO

* Alternative Pages
* Comparison Pages
* Knowledge Bases
* Programmatic SEO

## AI Tools GEO

* AI Directories
* AI Agents
* AI SaaS Products

---

# Research Papers

## Academic Research

* Retrieval-Augmented Generation (RAG)
* Search-Augmented LLMs
* AI Search Ranking

## Industry Research

* OpenAI Research
* Anthropic Research
* Google Research

---

# GEO Prompts

## GEO Audit Prompt

Analyze my website for GEO opportunities and explain how it can become more visible to AI search systems.

## FAQ Generation Prompt

Generate GEO-friendly FAQs based on this product page.

## Comparison Page Prompt

Generate a comprehensive comparison page between Product A and Product B.

---

# GEO Checklist

## Technical GEO

* Structured Data
* Sitemap.xml
* llms.txt
* Fast Loading
* Crawlability

## Content GEO

* FAQs
* Comparisons
* Alternatives
* Use Cases
* Entity Coverage

## Authority GEO

* Citations
* Mentions
* Reviews
* Backlinks

---

# Communities & Newsletters

## Communities

* Reddit
* Hacker News
* Indie Hackers

## Newsletters

* GEO News
* AI Search News
* SEO & AI Publications

---

# Contributing

Contributions are welcome.

If you know a useful GEO tool, case study, research paper, prompt, or resource, please open a Pull Request.

---

# Related Projects

* AIDeckly — Discover AI tools and growth opportunities.
* SubmitDeckly — Promote your AI product.
* ScoutDeckly — AI-powered growth intelligence.

---

# License

MIT
